...
func (m *manager) applyRuntimeEnvOnlyPersistPaths(newCfg *Config, paths []string) (map[string]struct{}, error) {
	...
	for _, path := range paths {
		...
		if !ok {
			return nil, missingRuntimeEnvError(path)
		}
		...
		if !setValueFromEnv(field, raw) {
			return nil, fmt.Errorf("%s cannot be set from environment variable %s", path, envName)
		}
		runtimeOnlyPaths[path] = struct{}{}
	}
	return runtimeOnlyPaths, nil
}
