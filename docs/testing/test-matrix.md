...
| `internal/transport/rpc` | JSON-RPC 方法注册 |
| `internal/modules/demo` | Todo service/repository/handler 行为 |
| `internal/modules/iam` | 组织、账号、登录刷新、MFA、邀请、密码重置、会话撤销、权限和审计行为 |
| `internal/modules/plugins` | manifest 加载、健康检查和代理行为 |
| `internal/modules/system` | 菜单、API、字典、参数、操作记录、配置、版本、媒体和服务状态行为 |
| `pkg/database` | 连接管理、事务辅助、重载 |
| `pkg/httpserver` | 启动、关闭、重载行为 |
| `pkg/rpcserver` | JSON-RPC handler、注册表、启动、关闭和重载行为 |
...
| `pkg/hostmetrics` | 主机 CPU、内存和磁盘采样结构 |
| `pkg/processx` | PID 存活和创建时间校验 |
| `types` | 响应和错误信封行为 |
