...
| 媒体库 | `system_media_categories` 和 `system_media_assets` 存储分类、上传文件和外链资源，二进制对象通过 `pkg/storage` 写入。 |

## 版本发布包

版本发布包参考 Gin-Vue-Admin 的版本管理工作流，但按本项目边界做了本地化：
...
媒体库参考 Gin-Vue-Admin 的“媒体库（上传下载）”页面，但按本项目的 storage/IAM 边界实现：
...
- `storage.enabled=false` 时仍可查看数据库中的媒体记录和导入外链；普通上传、本地文件下载和本地对象删除会返回 storage unavailable。
...
- 如果新增配置字段，同步更新 `configs/*.example.yaml`、`.env.example` 和 `docs/environment/configuration.md`。版本发布包没有新增配置开关；媒体库复用现有 storage 配置。
