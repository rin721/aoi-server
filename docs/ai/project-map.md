...
- System parity module: `internal/modules/system` follows the same
  `model -> repository -> service -> handler` flow for Gin-Vue-Admin-inspired
...
- API catalog entries expose route access mode (`public`, `authenticated`, or
  ...
  this mirrors GVA's public/private router split without renaming transport
  packages or storing duplicate middleware state.
- Login logs are a GVA-style admin page over existing IAM `auth.login` audit
  records; do not create a duplicate login-log table until there is a product
  requirement for different retention or fields.
- Login captcha is a GVA-style optional IAM workflow. `GET /api/v1/auth/captcha`
...
- Error logs are a GVA-style admin page over `system_operation_records` with
...
For Gin-Vue-Admin parity slices, keep the existing Go scaffold boundaries:
