...
登录后台后进入左侧菜单的 `安全`。页面会展示当前账号、组织、会话 ID、token
过期时间和 MFA 状态。点击 `生成密钥` 后，把 Secret 或 otpauth URL 录入验证器，
再填写 6 位验证码启用 MFA。

进入左侧菜单的 `会话` 可以查看当前组织范围内的会话。页面以分页对象读取
`GET /api/v1/orgs/{orgId}/sessions?scope=org`，支持按关键字、User ID、IP 和
状态筛选。发现不再可信的有效会话时，点击 `撤销` 让对应 refresh token 失效。
