...
Dockerfile 还包含 `web-build` 阶段，用 `web/aoi-web` 的 `pnpm generate` 生成 Go 可静态托管的后台产物，并复制到镜像内：

```text
/app/web/aoi-web/.output/public
```

`pnpm build` 可作为 Nuxt 构建检查；实际由 Go `/admin` 托管时必须保证 `.output/public/index.html` 存在。构建参数 `NUXT_PUBLIC_SHOW_DEMO_TODO=false` 默认隐藏 Demo Todo 导航入口。
