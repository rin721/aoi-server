...
const resolvedLabel = computed(() => props.label || t("imageTools.label"))
const helperText = computed(() => {
  if (errorText.value) {
    return translateImageToolMessage(errorText.value)
  }
...
async function waitForWorkbench(timeout = WORKBENCH_READY_TIMEOUT) {
  ...
  if (!workbenchRef.value) {
    throw new Error("imageTools.workbenchNotReady")
  }

  return workbenchRef.value
}

async function loadFile(file: File, successMessage = "imageTools.loaded") {
  if (props.disabled) {
    return
  }

  if (!isAcceptedImageFile(file, props.accept)) {
    handleError("imageTools.imageOnly")
    return
  }
...
async function exportToWebp(options: AoiImageClipboardExportOptions = {}) {
  if (!hasSource.value) {
    const message = "imageTools.chooseFirst"
...
function reset() {
  ...
  statusText.value = hasSource.value ? "imageTools.resetCrop" : ""
}
...
function updateResult(payload: AoiImageClipboardResult) {
  ...
  statusText.value = "imageTools.generated"
  errorText.value = ""
...
function onInputChange(event: Event) {
  ...
  if (files[0]) {
    void loadFile(
      files[0],
      files.length > 1 ? "imageTools.singleOnly" : undefined
    )
  }
  ...
  >
    ...
    >
      ...
      <span class="aoi-image-clipboard__drop-copy">
        <strong>{{ hasSource ? t("imageTools.replaceImage") : t("imageTools.chooseOrDrop") }}</strong>
        <span>{{ t("imageTools.pasteHint") }}</span>
      </span>
    </button>

    <div v-if="hasResult && result" class="aoi-image-clipboard__result">
      <img :src="result.objectUrl" :alt="t('imageTools.resultAlt')">
      <div class="aoi-image-clipboard__result-meta">
        ...
        <span v-if="result.sourceName">{{ t("imageTools.source", { name: result.sourceName }) }}</span>
      </div>
      <AoiActionBar class="aoi-image-clipboard__result-actions" size="sm">
        ...
        <AoiIconButton icon="trash-2" :label="t('imageTools.clearImage')" size="sm" :disabled="disabled" @click="clear" />
      </AoiActionBar>
    </div>
