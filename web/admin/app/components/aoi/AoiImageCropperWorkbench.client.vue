...
const primaryActionLabel = computed(() => props.confirmBehavior === "export-stay" ? t("imageTools.exportStay") : t("imageTools.exportUse"))
const statusLine = computed(() => {
  if (imageLoading.value) {
    return t("imageTools.loading")
  }

  if (isExporting.value) {
    return t("imageTools.exporting")
  }

  return translateImageToolMessage(statusText.value)
})
const resultSummary = computed(() => {
  ...
  return resultDirty.value ? t("imageTools.dirtySummary", { summary }) : summary
})
...
watch(selectedAspect, () => {
  applyAspectRatio()
  markResultDirty("imageTools.ratioDirty")
})

watch([qualityPercent, maxWidthValue, maxHeightValue], () => {
  markResultDirty("imageTools.exportDirty")
})
...
function waitForCropperReady(timeout = CROPPER_READY_TIMEOUT) {
  ...
  return Promise.race([
    ...
    new Promise<void>((_, reject) => {
      window.setTimeout(() => reject(new Error("imageTools.timeout")), timeout)
    })
  ])
}
...
function initializeCropper() {
  ...
  cropper = new Cropper(imageRef.value, {
    ...
    crop: (event) => {
      ...
      markResultDirty("imageTools.cropDirty")
    },
    cropBoxMovable: true,
    ...
    ready: () => {
      ...
      refreshCropperLayout()
      statusText.value = "imageTools.ready"
      resolveReadyPromise()
    },
    responsive: true,
    rotatable: true,
...
function markResultDirty(message = "imageTools.dirty") {
...
function reset() {
  ...
  result.value = null
  resultDirty.value = false
  statusText.value = "imageTools.reset"
  nextTick(applyAspectRatio)
}

function rotateBy(degrees: number) {
  cropper?.rotate(degrees)
  markResultDirty("imageTools.rotated")
}

function flipHorizontal() {
  flipX.value *= -1
  cropper?.scaleX(flipX.value)
  markResultDirty("imageTools.flipped")
}
...
function canvasToBlob(canvas: HTMLCanvasElement, quality: number) {
  return new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((blob) => {
      ...
      reject(new Error("imageTools.unsupported"))
    }, "image/webp", quality)
  })
}
...
async function waitForImageDecode() {
  ...
  if (!image) {
    throw new Error("imageTools.loadFirst")
  }

  if (!image.complete) {
    await new Promise<void>((resolve, reject) => {
      image.addEventListener("load", () => resolve(), { once: true })
      image.addEventListener("error", () => reject(new Error("imageTools.readFailed")), { once: true })
    })
  }
...
async function exportToWebp(options: AoiImageClipboardExportOptions = {}) {
  ...
  errorText.value = ""
  statusText.value = "imageTools.exporting"
  ...
  try {
    ...
    if (!cropperInstance) {
      throw new Error("imageTools.cropperNotReady")
    }
    ...
    if (!canvas) {
      throw new Error("imageTools.cropExportFailed")
    }
    ...
    resultDirty.value = false
    statusText.value = "imageTools.exported"
    errorText.value = ""
    emit("result", payload)
...
async function exportSafely() {
  try {
    ...
    handleError(error instanceof Error ? error.message : "imageTools.exportFailed")
    return undefined
  }
}
...
async function copyResult() {
  try {
    ...
    statusText.value = "imageTools.copied"
    errorText.value = ""

    return payload
  } catch (error) {
    handleError(error instanceof Error ? error.message : "imageTools.copyFailed")
    return undefined
  }
}
...
async function downloadResult() {
  try {
    ...
    statusText.value = "imageTools.downloaded"
    errorText.value = ""

    return payload
  } catch (error) {
    handleError(error instanceof Error ? error.message : "imageTools.downloadFailed")
    return undefined
  }
}
...
function handleError(message: string) {
  ...
  statusText.value = ""
  emit("error", translateImageToolMessage(message))
}

function translateImageToolMessage(message: string) {
  return message.startsWith("imageTools.") ? t(message) : message
}
...
function onImageError() {
  imageLoading.value = false
  handleError("imageTools.readRetry")
}
    ...
    <div class="aoi-image-cropper__workspace">
      ...
      >
        ...
        <div v-if="imageLoading" class="aoi-image-cropper__loading">
          ...
          <span>{{ t("imageTools.loading") }}</span>
        </div>
      </div>
      ...
      <aside class="aoi-image-cropper__panel">
        ...
        <div class="aoi-image-cropper__section">
          <span class="aoi-image-cropper__label">{{ t("imageTools.ratio") }}</span>
          <AoiButtonBox
            :model-value="String(selectedAspect)"
            :items="aspectItems"
            :aria-label="t('imageTools.cropRatio')"
            no-checkmark
        ...
        <AoiActionBar size="sm" :label="t('imageTools.transform')">
          <AoiIconButton icon="rotate-ccw" :label="t('imageTools.rotateLeft')" size="sm" @click="rotateBy(-90)" />
          <AoiIconButton icon="rotate-cw" :label="t('imageTools.rotateRight')" size="sm" @click="rotateBy(90)" />
          <AoiIconButton icon="flip-horizontal-2" :label="t('imageTools.flipHorizontal')" size="sm" :active="flipX < 0" @click="flipHorizontal" />
          <AoiIconButton icon="flip-vertical-2" :label="t('imageTools.flipVertical')" size="sm" :active="flipY < 0" @click="flipVertical" />
          <AoiIconButton icon="refresh-cw" :label="t('common.actions.reset')" size="sm" @click="reset" />
        </AoiActionBar>
