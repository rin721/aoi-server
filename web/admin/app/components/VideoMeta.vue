...
function formatRelativeDate(date: Date) {
  ...
  if (minutes < 60) {
    return t("common.dates.minutesAgo", { count: minutes })
  }

  const hours = Math.floor(minutes / 60)

  if (hours < 24) {
    return t("common.dates.hoursAgo", { count: hours })
  }

  const days = Math.floor(hours / 24)

  if (days < 30) {
    return t("common.dates.daysAgo", { count: days })
  }

  const months = Math.floor(days / 30)

  if (months < 12) {
    return t("common.dates.monthsAgo", { count: months })
  }

  return t("common.dates.yearsAgo", { count: Math.floor(months / 12) })
}
</script>
