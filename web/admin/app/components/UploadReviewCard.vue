...
const visibilityLabel = computed(() => {
  if (props.visibility === "public") {
    return t("common.states.public")
  }

  if (props.visibility === "unlisted") {
    return t("common.states.unlisted")
  }

  return t("common.states.private")
})
</script>
...
<template>
  <div class="upload-review-card">
    ...
    <h3>{{ props.title || t("common.states.unnamedDraft") }}</h3>
    <p>{{ props.description || t("upload.reviewFallbackDescription") }}</p>

    <dl class="upload-review-card__meta">
      <div>
        <dt>{{ t("upload.categoryLabel") }}</dt>
        <dd>{{ props.categoryName }}</dd>
      </div>
      <div>
        <dt>{{ t("upload.visibilityLabel") }}</dt>
        <dd>{{ visibilityLabel }}</dd>
      </div>
      <div>
        <dt>{{ t("common.states.draft") }}</dt>
