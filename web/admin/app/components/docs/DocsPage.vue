...
const docsNavigation = computed(() => localizeNavigation((navigation.value || []) as DocsNavigationItem[]))
const flatNav = computed(() => flattenNav(docsNavigation.value))
...
function localizeNavigation(items: DocsNavigationItem[]): DocsNavigationItem[] {
  const titleMap: Record<string, string> = {
    Components: t("docs.navGroups.components"),
    Docs: t("docs.navGroups.docs"),
    Project: t("docs.navGroups.project")
  }

  return items.map((item) => ({
    ...item,
    title: item.title ? titleMap[item.title] || item.title : item.title,
    children: item.children?.length ? localizeNavigation(item.children) : item.children
  }))
}
