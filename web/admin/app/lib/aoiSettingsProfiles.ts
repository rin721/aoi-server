...
type AoiSettingsProfileFieldDefinition = Omit<AoiSettingsProfileField, "depth" | "groupKey" | "labelKey" | "pageId">

export interface AoiSettingsProfileLocalization {
  fieldLabel?: (field: AoiSettingsProfileField) => string
  pageLabel?: (pageId: AoiSettingsProfilePageId) => string
  other?: string
  unset?: string
}
...
const PROFILE_FIELD_GROUP_KEYS: Record<string, string> = {
  "主题与颜色": "profileFields.groups.themeColor",
  "偏好": "profileFields.groups.preferences",
  "派生强度": "profileFields.groups.derivation",
  "外观形态": "profileFields.groups.appearance",
  "背景": "profileFields.groups.background",
  "弹幕": "profileFields.groups.danmaku",
  "规格单位": "profileFields.groups.spec",
  "动效与滚动": "profileFields.groups.motion",
  "路由进度": "profileFields.groups.routeProgress"
}

function getAoiSettingsProfileFieldToken(key: string) {
  return key.replace(/[^A-Za-z0-9]+/g, "_")
}

function getAoiSettingsProfileFieldGroupKey(group: string) {
  return PROFILE_FIELD_GROUP_KEYS[group] || "profileFields.groups.other"
}
...
export const AOI_SETTINGS_PROFILE_FIELDS: AoiSettingsProfileField[] = AOI_SETTINGS_PROFILE_FIELD_DEFINITIONS.map((field) => ({
  ...
  groupKey: getAoiSettingsProfileFieldGroupKey(field.group),
  labelKey: `profileFields.fields.${getAoiSettingsProfileFieldToken(field.key)}`,
...
export function createAoiSettingsProfileDiff(
  ...
  fieldKeys: string[],
  scope: AoiSettingsProfileScope,
  localization: AoiSettingsProfileLocalization = {}
  ...
  return normalizeAoiSettingsProfileFieldKeys(fieldKeys, scope, { fallbackToAll: false }).map((key) => {
    ...
    const before = formatAoiSettingsProfileValue(beforeValue, localization.unset)
    const after = formatAoiSettingsProfileValue(afterValue, localization.unset)
...
export function formatAoiSettingsProfileValue(value: unknown, unsetLabel = "未设置") {
  if (value === undefined) {
    return unsetLabel
  }
...
export function summarizeAoiSettingsProfileFields(
  fieldKeys: string[],
  scope: AoiSettingsProfileScope,
  localization: AoiSettingsProfileLocalization = {}
) {
  ...
  fields.forEach((key) => {
    const field = getAoiSettingsProfileField(key)
    const group = field
      ? localization.pageLabel?.(field.pageId) || PROFILE_PAGE_LABELS[field.pageId]
      : localization.other || "其它"
