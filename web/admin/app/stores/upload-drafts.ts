...
export const useUploadDraftStore = defineStore("uploadDrafts", () => {
  ...
  function validateDraft(draft: UploadDraft): UploadDraftValidation {
    ...
    if (draft.title.trim().length < 4) {
      missing.push("upload.validation.title")
    }

    if (!draft.source) {
      missing.push("upload.validation.file")
    }

    if (!draft.categorySlug) {
      missing.push("upload.validation.category")
    }

    if (draft.description.trim().length < 20) {
      warnings.push("upload.validation.description")
    }

    if (draft.tags.length < 2) {
      warnings.push("upload.validation.tags")
    }
