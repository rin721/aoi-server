...
Go 后端默认托管 `web/admin/.output/public` 到 `/admin`。如果要验证 Go 静态挂载，必须先运行 `pnpm generate`，确认 `.output/public/index.html` 存在。
...
- 需要由 Go 静态托管时，运行 `pnpm generate` 并确认 `web/admin/.output/public/index.html` 存在。
