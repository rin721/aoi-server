...
export function useAoiApi() {
  ...
  async function request<T>(endpoint: string, options: RequestOptions = {}): Promise<T> {
    try {
      ...
      const apiError = toAoiApiError(error, endpoint, t("common.errors.requestFailed"))
...
function toAoiApiError(error: unknown, endpoint: string, fallbackMessage: string): AoiApiErrorPayload {
  ...
  return {
    code,
    endpoint,
    message: responseError?.message || fetchError.message || fallbackMessage,
