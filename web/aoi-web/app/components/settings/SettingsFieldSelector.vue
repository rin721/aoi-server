...
function fieldLabel(field: AoiSettingsProfileField) {
  const depth = field.depth === "all" ? t("settings.profile.depthAll") : t("settings.profile.depthBasic")

  return `${t(field.labelKey)} · ${depth} · ${field.path}`
}
</script>
...
<template>
  <div class="settings-field-selector">
    <div class="settings-field-selector__toolbar">
      <div>
        <strong>{{ t("settings.profile.fieldSelector") }}</strong>
        <span>{{ t("settings.profile.selectedSummary", { count: props.modelValue.length, summary: props.summary }) }}</span>
      </div>
      <AoiActionBar size="sm" align="end">
        <AoiButton appearance="plain" intent="secondary" size="sm" icon="list-checks" @click="emit('selectAll')">
          {{ t("settings.profile.selectAll") }}
        </AoiButton>
