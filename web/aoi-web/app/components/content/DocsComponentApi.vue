...
import { getLocalizedAoiComponentCategories, getLocalizedAoiComponentDocs } from "~/data/aoiComponentDocs"
import type { AoiComponentDocApiRow, AoiComponentDocCategory } from "~/types/docs"
...
const { t } = useI18n()
const docsLocale = useAoiDocsLocale()
const docs = computed(() => getLocalizedAoiComponentDocs(docsLocale.value, props.category))
const categories = computed(() => getLocalizedAoiComponentCategories(docsLocale.value))
const categoryMeta = computed(() => props.category
  ? categories.value.find((item) => item.id === props.category)
  : undefined)

function emptyText(rows: AoiComponentDocApiRow[]) {
  return rows.length ? "" : t("docs.componentApi.none")
}

function categoryTitle(category: AoiComponentDocCategory) {
  return categories.value.find((item) => item.id === category)?.title || category
}
</script>
  ...
  <section class="docs-component-api">
    ...
    >
      <div class="docs-component-api__item-header">
        <div>
          <p class="docs-component-api__eyebrow">{{ categoryTitle(doc.category) }}</p>
          <h3>{{ doc.name }}</h3>
          <p>{{ doc.description }}</p>
        </div>
      ...
      <div class="docs-component-api__usage">
        <strong>{{ t("docs.componentApi.usage") }}</strong>
        <p>{{ doc.usage }}</p>
      </div>

      <div class="docs-component-api__tables">
        <section>
          <h4>{{ t("docs.componentApi.props") }}</h4>
          <p v-if="emptyText(doc.props)" class="docs-component-api__empty">{{ emptyText(doc.props) }}</p>
          <table v-else>
            <thead>
              <tr>
                <th>{{ t("docs.componentApi.name") }}</th>
                <th>{{ t("docs.componentApi.type") }}</th>
                <th>{{ t("docs.componentApi.default") }}</th>
                <th>{{ t("docs.componentApi.description") }}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <section>
          <h4>{{ t("docs.componentApi.slots") }}</h4>
