...
- Go serves the static admin console from `web/aoi-web/.output/public` at `/admin`. Use `pnpm generate` to produce that directory; `pnpm build` is a build check and does not replace static generation.
...
- Before handing off Go-hosted admin changes, run `pnpm generate` and confirm `web/aoi-web/.output/public/index.html` exists.
